  ...
  function Html5Hlsjs(source, tech) {
    ...
    function _relayQualityChange() {
      ...
      if (isAuto) {
        ...
        var selectedTrack;
        for (
          selectedTrack = qualityLevels.length - 1;
          selectedTrack >= 0;
          selectedTrack--
        ) {
    ...
    function reBindHlsJs() { 
      try {
        Object.keys(_hls).forEach((key) => {
          if (_hls[key] &&_hls[key].hls === null) { 
            _hls[key].hls = _hls;
          }
        })
      } catch(e) { 
        console.error(`videpojs-hlsjs: Error on reBindHlsJs. message: ${e.message || e}`);
      }
    }
    ...
    function _isSameTextTrack(track1, track2) {
      return (
        _getTextTrackLabel(track1) === _getTextTrackLabel(track2) &&
    ...
    function _updateSelectedTextTrack() {
      ...
      for (var k = 0; k < hlsjsTracks.length; k++) {
        if (
          hlsjsTracks[k].kind === "subtitles" ||
          hlsjsTracks[k].kind === "captions"
        ) {
          hlsjsTracks[k].mode =
            activeTrack && _isSameTextTrack(hlsjsTracks[k], activeTrack)
              ? "showing"
              : "disabled";
        }
      }
    ...
    function _initHlsjs() {
      var hlsjsConfigRef =
        (_player.srOptions_ && _player.srOptions_.hlsjsConfig) ||
        tech.options_.hlsjsConfig;
      ...
      var captionConfig =
        (_player.srOptions_ && _player.srOptions_.captionConfig) ||
        tech.options_.captionConfig;
      if (captionConfig) {
      ...
      _hls.on(Hlsjs.Events.LEVEL_LOADED, function (event, data) {
        ...
        if (_hlsjsConfig.liveSyncDuration) {
          ...
          _edgeMargin =
            _hlsjsConfig.liveSyncDurationCount * data.details.targetduration;
        }
    ...
    this.dispose = function () {
      ...
      _player
        .textTracks()
        .removeEventListener("change", _updateSelectedTextTrack);
      _uiTextTrackHandled = false;
      _player
        .audioTracks()
        .removeEventListener("change", _updateSelectedAudioTrack);
